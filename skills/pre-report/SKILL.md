...
4. `input/stt/` 에 STT 파일이 있으면 읽는다.
   ...
   - **STT는 보조 자료**로만 사용한다. STT에서 강의노트·교재와 다른 실험 변형(전압 설정 변경, 추가 소자 병렬/직렬 결합 등)이 발견되더라도:
     - 본문에 STT 발화를 직접 인용하지 않는다 (예: "STT 교차검증" 박스 금지).
     - 변형 실험에 대한 별도 Table을 추가하지 않는다 (예: Table X.Nb 같은 변형 표 금지).
     - 강의노트 기준으로 통일하여 작성한다.

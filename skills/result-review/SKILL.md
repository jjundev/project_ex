...
### Step 1a: 고찰 위치 검증 (필수, 구조 우선)

품질 검토보다 먼저 `# 고찰`의 *위치*를 확인한다. `# 고찰`은 보고서의 *마지막* top-level `#` 섹션이어야 한다 (그 뒤에 다른 `# ` 섹션이 없어야 한다). `# 연습 문제` 섹션이 있으면 `# 고찰`이 그 *뒤*에 와야 한다. `# 고찰`이 `# 실험 결과`와 `# 연습 문제` 사이 등 잘못된 위치에 있으면 구조 FAIL로 즉시 차단한다 (위반 시 어느 섹션 사이에 끼어 있는지 줄 위치를 인용).
...
### 고찰 위치
- 판정: PASS 또는 FAIL (# 고찰이 마지막 top-level 섹션이며 # 연습 문제 뒤인지, 위반 시 끼어든 위치)

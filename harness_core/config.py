...
# extract_pass_sections가 review 파일의 H3에서 PASS 섹션 이름을 추출할 때
# 보고서에 실제 존재하는 섹션만 인정하기 위한 화이트리스트.
# review 파일에는 "경계 침범 체크", "STT 충돌 처리 체크", "발견된 문제점" 같은
# 검토 카테고리 H3도 함께 있으므로, 그것들은 이 집합에 없어 자동 필터링된다.
REPORT_SECTIONS: frozenset[str] = frozenset({
    "실험 목적",
    "실험 준비물",
    "실험 이론",
    "예상 결과 값",
...
# 결과보고서 top-level(`# `) 섹션의 정규 순서. normalize_result_section_order가
# 제목 다음의 알려진 섹션을 이 순서로 재배치한다 (REPORT_SECTIONS는 순서 없는
# frozenset 이므로 순서 강제용 튜플을 별도로 둔다).
RESULT_SECTION_ORDER: tuple[str, ...] = ("실험 결과", "연습 문제", "고찰")
